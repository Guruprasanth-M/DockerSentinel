#!/bin/bash
# restore.sh — Restore HostSpectra from a backup archive
set -euo pipefail
...
if [ $# -lt 1 ]; then
    ...
    echo "Example: $0 backups/hostspectra_backup_20240115_143301.tar.gz"
    exit 1
fi
...
if docker compose exec -T db \
    pg_dump -U "${DB_USER:-hostspectra}" "${DB_NAME:-hostspectra}" > "$ROLLBACK_DB_DUMP" 2>/dev/null; then
...
for i in $(seq 1 30); do
    if docker compose exec -T db pg_isready -U "${DB_USER:-hostspectra}" > /dev/null 2>&1; then
        break
    fi
    sleep 1
done
...
if [ -n "$SQL_DUMPS" ]; then
    ...
    if [ -n "$SQL_FILE" ] && [ -s "$SQL_FILE" ]; then
        if docker compose exec -T db \
            psql -U "${DB_USER:-hostspectra}" "${DB_NAME:-hostspectra}" < "$SQL_FILE" > /dev/null 2>&1; then
